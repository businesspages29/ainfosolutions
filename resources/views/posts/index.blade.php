    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="flex justify-between pt-3 pb-3">
                ...
                <x-primary-link href="{{ route('posts.create') }}" class="float-right">
                    {{ __('Add') }}
                </x-primary-link>
            </div>
            <x-tables.table>
                ...
                <x-slot name="body">
                    @forelse ($records as $key => $record)
                        <x-tables.row>
                            ...
                            <x-tables.cell>{{ $record->title }}</x-tables.cell>
                            ...
                            <x-tables.cell class="text-center">
                                <form action="{{ route('posts.destroy', $record->id) }}" method="POST"
                                    class="flex gap-2">
                                    <x-primary-link href="{{ route('posts.show', $record->id) }}">Show</x-primary-link>
                                    <x-secondary-link
                                        href="{{ route('posts.edit', $record->id) }}">Edit</x-secondary-link>
                                    @csrf
                                    @method('DELETE')
